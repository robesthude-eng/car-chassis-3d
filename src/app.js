...
import { createSceneMaterials, COLOR_ZONES } from "./materials.js";
...
async function bootstrapScene() {
  ...
  scene.fog = new THREE.FogExp2(0x2b3038, 0.02);
  ...
  _envPanel(0xffffff, 10.0, 7.5, 0.12, 3.2, 0, 3.7, 0.8);
  _envPanel(0xffffff, 6.0, 3.2, 0.12, 6.5, -2.4, 3.7, -1.2);
  _envPanel(0xdae8ff, 4.0, 0.12, 3.0, 7.0, -7.2, 2.4, 0);
  _envPanel(0xffe6c8, 2.8, 0.12, 2.6, 6.0, 7.2, 2.2, 0);
  _envPanel(0xc7d8f2, 1.8, 6.0, 2.4, 0.12, 0, 2.2, -7.2);
  _envPanel(0x6b7480, 0.9, 16, 0.12, 16, 0, -1.2, 0);
  ...
  const ambientLight = new THREE.AmbientLight(0xd8e2f0, 0.25);
  scene.add(ambientLight);

  const hemiLight = new THREE.HemisphereLight(0xaec4dd, 0x2b2724, 0.55);
  scene.add(hemiLight);

  const dirLight1 = new THREE.DirectionalLight(0xfff5eb, 3.0);
  ...
  const gridHelper = new THREE.GridHelper(20, 20, 0x3a4550, 0x2d3540);
  ...
  function updateSimulation(time, deltaSec) {
    ...
    suspensionCorners.forEach((sc, idx) => {
      ...
      if (cm.isF) {
        ...
        /* Все размеры стойки — от расчётной длины кинематики */
        const L = cm.strutLen; // полная длина стойки от хомута до верхней опоры
        const L_body = L * 0.52; // корпус стойки занимает ~52% длины
        const L_spring = L * 0.48; // пружина ~48%
        const L_rod = L * 0.32; // шток ~32%

        /* Корпус стойки: идёт от хомута вверх */
        st.strutBody.scale.y = L_body / 0.28;
        st.strutBody.position.y = L_body * 0.5;

        /* Нижняя тарелка пружины — у верхнего края корпуса */
        st.lowerSpringPerch.position.y = L_body;

        /* Пружина: стоит на нижней тарелке */
        st.springMesh.position.y = L_body;
        st.springMesh.scale.y = L_spring / 0.32;
        st.upperSeat.position.y = L_body + L_spring;

        /* Шток: от верхней тарелки пружины до верхней опоры */
        st.pistonRod.scale.y = L_rod / 0.32;
        st.pistonRod.position.y = L - L_rod * 0.5;

        /* Верхняя опора и внутреннее кольцо подшипника */
        st.topMount.position.y = L;
        st.bearingInner.position.y = L - 0.025;

        /* Наружная обойма стоит в стакане кузова */
        ...
        /* Отбойник сжимается на bumpHit */
        const bsLen = Math.max(0.026, 0.075 - cm.bumpHit * 0.75);
        st.bumpStop.scale.y = bsLen / 0.075;
        st.bumpStop.position.y = L - 0.05 - bsLen * 0.5;

        /* Пыльник: закрывает открытую часть штока */
        const bootTop = L - 0.05 - bsLen;
        const bootBot = L_body + 0.01;
        const bootLen = Math.max(0.03, bootTop - bootBot);
        st.dustBoot.scale.y = bootLen / 0.18;
        st.dustBoot.position.y = bootBot + bootLen * 0.5;
        ...
        rl.springSeat.position.y = 0;
        rl.springTopSeat.position.y = springLenVis;

        /* Задний амортизатор: корпус масштабируется, шток выдвигается */
        const dmpLenVis = aimGroup(rl.damperGroup, cm.dmpBot, cm.dmpTop);
        const dmpScale = clamp(dmpLenVis / 0.46, 0.55, 1.3);
        rl.damperBody.scale.y = dmpScale;
        rl.damperBody.position.y = 0.13 * dmpScale;
        const dmpRodLen = Math.max(0.1, dmpLenVis - 0.18);
        rl.pistonRod.scale.y = dmpRodLen / 0.26;
        rl.pistonRod.position.y = dmpLenVis - dmpRodLen * 0.5;
        rl.damperTopMount.position.y = dmpLenVis;
        rl.damperLowerEye.position.y = 0;
  ...
  btnColorCode.addEventListener("click", () => {
    ...
    COLOR_ZONES.forEach((zone) => {
      const mat = materials[zone.key];
      if (!mat || !mat.color) return;
      mat.color.setHex(state.isColorCoded ? zone.coded : zone.neutral);
