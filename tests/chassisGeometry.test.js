...
test("продольный рычаг не заходит в бочку колеса", () => {
	const trOut = CHASSIS_GEOMETRY.rearCarrier.trOut;
	assert.ok(
		trOut[0] <= -0.11,
		`ухо продольного рычага x=${trOut[0]} заходит под плоскость обода`,
	);
});
